...
sudo apt update
sudo apt upgrade
sudo apt install build-essential procps curl file git vim linux-modules-extra-raspi net-tools
```
...
sudo hostnamectl set-hostname k8s-n3.local
```
...
sudo apt-get update
sudo apt-get install -y kubelet kubeadm kubectl docker-ce containerd.io
...
## join node
```Bash
sudo kubeadm join k8s-m1.local:6443 --token k2jq22.pr5j9tiva6yvarkl \
